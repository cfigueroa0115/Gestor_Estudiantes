...
export function StudentRequestFormModal({
  isOpen,
  onClose,
  onSuccess,
}: StudentRequestFormModalProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      requiere_escalar: undefined as unknown as boolean,
      area_escalar: null,
    },
  });
  ...
  const onSubmit = async (data: StudentRequestInput) => {
    ...
    // Set conditional fields to null when not applicable
    const payload: StudentRequestInput = {
      ...data,
      solicitud_academica:
        data.tipo_solicitud === 'Académico' ? data.solicitud_academica : null,
      solicitud_financiera:
        data.tipo_solicitud === 'Financiero' ? data.solicitud_financiera : null,
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast(
          result.error || 'No se pudo registrar la solicitud. Intente nuevamente.',
          'error'
        );
  ...
  // When tipo_solicitud changes, clear conditional fields
  const handleTipoSolicitudChange = (
    e: React.ChangeEvent<HTMLSelectElement>
  ) => {
    const value = e.target.value as StudentRequestInput['tipo_solicitud'];
    setValue('tipo_solicitud', value);
    setValue('solicitud_academica', null);
    setValue('solicitud_financiera', null);
  };

  // When requiere_escalar changes, clear area_escalar if No
  const handleRequiereEscalarChange = (
    e: React.ChangeEvent<HTMLSelectElement>
  ) => {
    const value = e.target.value === 'true';
    setValue('requiere_escalar', value);
    if (!value) {
      setValue('area_escalar', null);
    }
  };
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Formulario de solicitud estudiantil"
    >
      <div
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <fieldset className="space-y-4">
            <legend className="text-sm font-semibold text-gris-700">
              Datos del estudiante
            </legend>

            {/* Fecha solicitud (readonly) */}
            <div>
              <label
              ...
              <input
                ...
                type="text"
                readOnly
                className="w-full rounded-lg border border-gris-300 bg-gris-50 px-3 py-2 text-sm text-gris-600"
          ...
          <fieldset className="space-y-4">
            ...
            {/* Solicitud Académica (condicional) */}
            {tipoSolicitud === 'Académico' && (
              <div>
                <label
                  htmlFor="solicitud_academica"
                  className="mb-1 block text-sm font-medium text-gris-700"
                >
                  Solicitud académica
                </label>
                <select
                  id="solicitud_academica"
                  className={`w-full rounded-lg border px-3 py-2 text-sm outline-none transition-colors focus:border-aguamarina-500 focus:ring-1 focus:ring-aguamarina-500 ${
                    errors.solicitud_academica
                      ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                      : 'border-gris-300'
                  }`}
                  onChange={(e) =>
                    setValue(
                      'solicitud_academica',
                      e.target.value as StudentRequestInput['solicitud_academica']
                    )
                  }
                  value={watch('solicitud_academica') || ''}
                  disabled={isSubmitting}
                >
                  <option value="" disabled>
                    Seleccione una opción
                  </option>
                  {SOLICITUD_ACADEMICA_OPTIONS.map((option) => (
                    <option key={option} value={option}>
                      {option}
                    </option>
                  ))}
                ...
                {errors.solicitud_academica && (
                  <p className="mt-1 text-xs text-red-600">
                    {errors.solicitud_academica.message}
                  </p>
                )}
              </div>
            )}

            {/* Solicitud Financiera (condicional) */}
            {tipoSolicitud === 'Financiero' && (
              <div>
                <label
                  htmlFor="solicitud_financiera"
                  className="mb-1 block text-sm font-medium text-gris-700"
                >
                  Solicitud financiera
                </label>
                <select
                  id="solicitud_financiera"
                  className={`w-full rounded-lg border px-3 py-2 text-sm outline-none transition-colors focus:border-aguamarina-500 focus:ring-1 focus:ring-aguamarina-500 ${
                    errors.solicitud_financiera
                      ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                      : 'border-gris-300'
                  }`}
                  onChange={(e) =>
                    setValue(
                      'solicitud_financiera',
                      e.target.value as StudentRequestInput['solicitud_financiera']
                    )
                  }
                  value={watch('solicitud_financiera') || ''}
                  disabled={isSubmitting}
                >
                  <option value="" disabled>
                    Seleccione una opción
                  </option>
                  {SOLICITUD_FINANCIERA_OPTIONS.map((option) => (
                    <option key={option} value={option}>
                      {option}
                    </option>
                  ))}
                ...
                {errors.solicitud_financiera && (
                  <p className="mt-1 text-xs text-red-600">
                    {errors.solicitud_financiera.message}
                  </p>
                )}
              </div>
            )}
          ...
          <fieldset className="space-y-4">
            <legend className="text-sm font-semibold text-gris-700">
              Descripción y escalamiento
            </legend>

            {/* Descripción solicitud */}
            <div>
              <label
                htmlFor="descripcion_solicitud"
                className="mb-1 block text-sm font-medium text-gris-700"
              >
              ...
              <textarea
                ...
                rows={4}
                maxLength={1200}
                placeholder="Describa la solicitud del estudiante..."
                className={`w-full resize-none rounded-lg border px-3 py-2 text-sm outline-none transition-colors focus:border-aguamarina-500 focus:ring-1 focus:ring-aguamarina-500 ${
              ...
              <p
                className={`mt-1 text-xs ${
                  caracteresRestantes < 0 ? 'text-red-600' : 'text-gris-500'
                }`}
              >
              ...
              {errors.descripcion_solicitud && (
                <p className="mt-1 text-xs text-red-600">
                  {errors.descripcion_solicitud.message}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <select
                id="requiere_escalar"
                className={`w-full rounded-lg border px-3 py-2 text-sm outline-none transition-colors focus:border-aguamarina-500 focus:ring-1 focus:ring-aguamarina-500 ${
                  errors.requiere_escalar
                    ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                    : 'border-gris-300'
                }`}
                onChange={handleRequiereEscalarChange}
                value={
              ...
              {errors.requiere_escalar && (
                <p className="mt-1 text-xs text-red-600">
                  {errors.requiere_escalar.message}
                </p>
              )}
            </div>

            {/* Área a escalar (condicional) */}
            {requiereEscalar === true && (
              <div>
                <label
                  htmlFor="area_escalar"
                  className="mb-1 block text-sm font-medium text-gris-700"
                >
                  Área a escalar
                </label>
                <select
                  ...
                  onChange={(e) =>
                    setValue(
                      'area_escalar',
                      e.target.value as StudentRequestInput['area_escalar']
                    )
                  }
                  value={watch('area_escalar') || ''}
                  disabled={isSubmitting}
                >
                  <option value="" disabled>
                    Seleccione un área
                  </option>
                  {AREA_ESCALAR_OPTIONS.map((option) => (
                    <option key={option} value={option}>
                      {option}
                    </option>
                  ))}
